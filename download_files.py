...
# loading metadata
# converting master annotation file to dataframe
...
def clustering_calls(calls_df, analysis_window_sec=5.0, max_cluster_span_sec=10.0):
    ...
    for soundfile, group_df in calls_df.groupby("Soundfile"):
        ...
        for i in range(1, len(df)):
            ...
            # starting new cluster if maximum span is exceeded
            else:
        ...
        # adding final cluster after checking all calls in file 
        clusters.append(pd.DataFrame(current_cluster))

        for cluster_id, cluster_df in enumerate(clusters):
            subpops = cluster_df["Subpopulation"].unique()
            # excluding clusters that include more than one subpopulation 
            if len(subpops) > 1:
            ...
            # idenitfying mean of cluster by avergaing across midpoints of calls in cluster
            cluster_midpoint = cluster_df["midpoint"].mean()
            # building one row per cluster for new dataframe 
    ...
    # returning final clusters and excluded ones in separate dataframes
...
# ensuring that clusters that span over 10 seconds are removed  
deduplicated_df = deduplicated_df[
    (deduplicated_df["FileEndSec"] - deduplicated_df["FileBeginSec"]) <= 10.0
].copy()

# defining function to get sampling rate if standard readers (librosa) can't parse the file 
...
# creating new dataframe with clusters instead of calls
calls, excluded = clustering_calls(deduplicated_df, analysis_window_sec=5.0, max_cluster_span_sec=10.0)

# data exploration 
# checking how distribution of subpopulations across providers
confound_check = pd.crosstab(calls['Subpopulation'], 
                             calls['Provider'])
print(confound_check)

# visualising distribution of subpopulations across providers
# creating colour mapping for providers 
providers = sorted(calls["Provider"].unique())
palette = sns.color_palette("tab20", n_colors=len(providers))
provider_color_map = dict(zip(providers, palette))

# plotting heatmap of raw counts 
plt.figure(figsize=(10, 6))
sns.heatmap(confound_check, annot=True, fmt="d", cmap="YlGnBu", cbar=True, linewidths=0.5)
plt.title(
    "Subpopulation vs. Provider (Raw Counts)", fontsize=14, fontweight="bold"
)
plt.xlabel("Provider", fontsize=12)
plt.ylabel("Subpopulation", fontsize=12)
plt.tight_layout()
plt.savefig("subpopulation_provider_heatmap.png", dpi=150)
plt.show()


# plotting barchart for proportion representaion 
confound_prop = (
    pd.crosstab(
        calls["Subpopulation"], calls["Provider"], normalize="index")
    * 100)

# ensure column order matches the provider list used for the color map
confound_prop = confound_prop[providers]
bar_colors = [provider_color_map[p] for p in confound_prop.columns]

ax = confound_prop.plot(
    kind="bar", stacked=True, figsize=(11, 6), color=bar_colors, edgecolor="black"
)
plt.title(
    "Provider Composition per Subpopulation (%)", fontsize=14, fontweight="bold"
)
plt.xlabel("Subpopulation", fontsize=12)
plt.ylabel("Percentage (%)", fontsize=12)
plt.xticks(rotation=0)
plt.legend(title="Provider", bbox_to_anchor=(1.05, 1), loc="upper left")
plt.tight_layout()
plt.savefig("subpopulation_provider_stacked_bar.png", dpi=150)
plt.show()

# grouping calls by subpopulation 
files_by_subpop = {
                provider: list(group["Soundfile"].unique())
                for provider, group in calls.groupby("Subpopulation")
                    }

rotation_order = []

# alternating soundfiles across subpopulations so downloads are balanced if interrupted
for files_tuple in zip_longest(*files_by_subpop.values()):
    for f in files_tuple:
        if f is not None:  
            rotation_order.append(f)

# grouping calls by the files they belong to 
calls_by_file_dict = dict(list(calls.groupby("Soundfile")))
...
calls["ExportFilename"] = [
    f"{row['Subpopulation']}/clip_{row['Soundfile'].replace('.', '_')}_{row['cluster_id']}.wav" 
    for idx, row in calls.iterrows()
]

# creating download loop
for soundfile in rotation_order:
    
    group_df = calls_by_file_dict[soundfile]
    ...
    try: 
        # naming temp file to match the cloud file's original extension 
        ...
        # confiriming download complete before further processing 
        ...
        if temp_raw_path and os.path.exists(temp_raw_path):
            ...
            # falling back to ffprobe if standard readers can't parse the file
            except Exception: 
...
# saving final cluster metadata table 
metadata_export_path = os.path.join(local_data_dir, "downloaded_calls_metadata.csv")
calls.to_csv(metadata_export_path, index = False) 
print(f"Saved matched metadata table to {metadata_export_path}")

# renamming clips with file type mentioned twice 
for root, dirs, files in os.walk(local_data_dir):
    for fname in files:
        if fname.endswith(".wav"):
            new_name = re.sub(r"_(flac|wav)_(\d+)\.wav$", r"_\2.wav", fname)
            if new_name != fname:
